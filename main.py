...
def no_projects(message):
    bot.send_message(message.chat.id, 'У тебя пока нет заданий!\nМожешь добавить их с помошью команды /new_task')
...
attributes_of_projects = {'Имя проекта' : ["Введите новое имя задания", "project_name"],
                          "Описание" : ["Введите новое описание задания", "description"],
...
def start_command(message):
    bot.send_message(message.chat.id, """Привет! Я бот-менеджер заданий!
...
/new_task - используй для добавления нового проекта
....
...
def name_project(message):
    ...
    data = [user_id, name]
    bot.send_message(message.chat.id, "Введите X (это баг, скоро пофиксим)")
    bot.register_next_step_handler(message, link_project, data=data)

def link_project(message, data):
    data.append(message.text)
    statuses = [x[0] for x in manager.get_statuses()] 
    bot.send_message(message.chat.id, "Введите текущий статус задания", reply_markup=gen_markup(statuses))
...
def callback_project(message, data, statuses):
    ...
    bot.send_message(message.chat.id, "Задание сохранено")
...
def skill_handler(message):
    ...
    if projects:
        ...
        bot.send_message(message.chat.id, 'Выбери задание для которого нужно выбрать навык', reply_markup=gen_markup(projects))
...
def skill_project(message, projects):
    ...
    if project_name not in projects:
        bot.send_message(message.chat.id, 'У тебя нет такого задания, попробуй еще раз!) Выбери задание для которого нужно выбрать навык', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, skill_project, projects=projects)
    else:
...
def set_skill(message, project_name, skills):
    ...
    bot.send_message(message.chat.id, f'Навык {skill} добавлен к заданию {project_name}')
...
def delete_project(message, projects):
    ...
    if project not in projects:
        bot.send_message(message.chat.id, 'У тебя нет такого задания, попробуй выбрать еще раз!', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, delete_project, projects=projects)
        return
    project_id = manager.get_project_id(project, user_id)
    manager.delete_project(user_id, project_id)
    bot.send_message(message.chat.id, f'Задание {project} удалено!')
...
def update_project(message):
    ...
    if projects:
        projects = [x[2] for x in projects]
        bot.send_message(message.chat.id, "Выбери задание, которое хочешь изменить", reply_markup=gen_markup(projects))
...
def update_project_step_2(message, projects):
    ...
    if message.text == cancel_button:
        ...
        return
    if project_name not in projects:
        bot.send_message(message.chat.id, "Что-то пошло не так!) Выбери задание, которое хочешь изменить еще раз:", reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, update_project_step_2, projects=projects )
        return
    bot.send_message(message.chat.id, "Выбери, что требуется изменить в задании", reply_markup=gen_markup(attributes_of_projects.keys()))
